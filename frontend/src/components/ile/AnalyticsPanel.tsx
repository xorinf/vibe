        ...
        {loading && !data ? (
          ...
          <div className="rounded-md border border-rose-200 bg-rose-50 px-3 py-2 text-xs text-rose-700">
            ...
            <Button
              ...
              variant="ghost"
              onClick={refresh}
              className="ml-2 h-6 text-rose-700 hover:bg-rose-100"
            >
              Retry
            </Button>
          </div>
  ...
  return (
    <li className="rounded-md border border-slate-200 bg-white">
      ...
      >
        ...
        <div className="flex shrink-0 flex-col items-end text-[10px] text-slate-500">
          <span className="font-semibold text-slate-700">{pct}%</span>
          {student.errorCount > 0 && (
            <span className="text-rose-600">{student.errorCount} err</span>
          )}
          {student.interactionCount > 0 && (
            <span className="text-slate-500">{student.interactionCount} act</span>
          )}
        </div>
      </button>
      {expanded && student.events.length > 0 && (
        <div className="border-t bg-slate-50 px-3 py-2">
          {student.resumePoint && (
            <p className="mb-1.5 flex items-center gap-1.5 text-[10px] text-slate-600">
          ...
          <ol className="space-y-1 text-[10px] font-mono text-slate-600">
            {student.events.slice(-12).map((e, idx) => (
              <li key={idx} className="flex items-start gap-1.5">
                <span className="mt-0.5 text-slate-400">
                  {new Date(e.receivedAt).toLocaleTimeString()}
                </span>
                <span className="rounded bg-slate-200 px-1 text-[9px] uppercase text-slate-700">
                  {e.kind}
                </span>
                {e.data != null && (
                  <span className="truncate text-slate-500">
