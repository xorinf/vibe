 ...
 *
 * Originally this module also exported a `useIleGeneration` hook that
 * owned the SSE transport. That hook has been removed — the actual
 * streaming goes through `useIleEditor.startEditStream` and
 * `useIleContextGeneration.start`. The type is kept here because it is
 * the canonical "stream state" shape consumed by the editor, the
 * preview, and the inspector drawer (see other modules in this folder).
 */
