        ...
        {hasExperience && (
          <>
            ...
            <Button
              ...
              className="h-7 w-7 p-0 text-emerald-600 hover:text-emerald-800 disabled:opacity-40"
            >
            ...
            <Button
              ...
              className="h-7 w-7 p-0 text-rose-600 hover:text-rose-800 disabled:opacity-40"
            >
          ...
          <div className="mt-2 flex flex-wrap items-center gap-1.5">
            {onAccept && (
              <button
                type="button"
                onClick={onAccept}
                className="inline-flex items-center gap-1 rounded-full border border-emerald-200 bg-emerald-50 px-2 py-0.5 text-[10px] font-medium text-emerald-700 transition-colors hover:bg-emerald-100"
            ...
            {onReject && (
              <button
                type="button"
                onClick={onReject}
                className="inline-flex items-center gap-1 rounded-full border border-rose-200 bg-rose-50 px-2 py-0.5 text-[10px] font-medium text-rose-700 transition-colors hover:bg-rose-100"
                title="Revert to the previous version"
              >
            ...
            {onRetry && (
              <button
                type="button"
                onClick={onRetry}
                className="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-white px-2 py-0.5 text-[10px] font-medium text-slate-700 transition-colors hover:bg-slate-50"
...
function StreamFooter({ state }: { state: IleEditorState }) {
  ...
  return (
    <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 border-t border-slate-100 px-1 pt-2 text-[10px] text-slate-500">
      {provider && (
        <span className="inline-flex items-center gap-1">
          <span className="rounded bg-slate-100 px-1.5 py-0.5 font-mono text-[9px] uppercase tracking-wider text-slate-600">
            {provider}
          </span>
        </span>
      )}
      {model && (
        <span className="inline-flex items-center gap-1 truncate font-mono text-[10px] text-slate-700" title={model}>
          {model}
        </span>
      )}
      {tokens != null && (
